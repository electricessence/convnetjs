...
export class Brain implements Brain.Options
{
	...
	epsilon:number; // controls exploration exploitation trade-off. Should be annealed over time
	...
	constructor(public num_states, public num_actions, options:Brain.Options = {})
	{
		...
		let tdtrainer_options:Trainer.Options = {
			learning_rate: 0.01,
			momentum: 0.0,
			batch_size: 64,
			l2_decay: 0.01
	...
	random_action():number
	{
		...
		{
			return Random.integer(0, this.num_actions);
		}
		else
		{
			// okay, lets do some fancier sampling:
			const p = Random.float(0, 1.0);
			let cumprob = 0.0;
	...
	getNetInput(xt:number[]):number[]
	{
		...
		let w = xt.slice(); // start with current state
		...
		for(let k = 0; k<this.temporal_window; k++)
		{
			...
			const action1ofk = initialize<number>(this.num_actions);
	...
	backward(reward:number)
	{
		...
		if(this.forward_passes>this.temporal_window + 1)
		{
			...
			else
			{
				...
				const ri = Random.integer(0, this.experience_size);
...
export module Brain {
	export interface Options
	{
		...
		tdtrainer_options?:Trainer.Options;
